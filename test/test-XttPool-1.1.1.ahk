...
class test_XttPool_110 {
    static Call() {
        ...
        this.lv := g.Add('ListView', 'w200 r4 -Hdr -Multi Section', [ '', '' ])
...
=
        ...
        WinMoveBottom(this.lv.Hwnd)
        this.lv.ModifyCol(1, 160)
        this.lv.ModifyCol(2, 30)
        g.Show()
        this.map := Map()
        ...
        infotip.AddControlRect('Available', 'It`'s okay if there are 0 available; the ``XttPool`` method call will automatically create a new ``Xtooltip``.', this.available)
    ...
    static HClickButtonRecallSelected(*) {
        if row := this.lv.GetNext() {
            n := Number(this.lv.GetText(row, 2))
            this.map.Get(n).Call()
            this.map.Delete(n)
            this.lv.Delete(row)
            this.available.Text := 'Available: ' pool.Length
